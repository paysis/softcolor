...
def soft_color_erosion(multivariate_image, structuring_element, fuzzy_implication_function):
    ...
    for i in range(multivariate_image.shape[0]):
        ...
        se_end_i = max(sz_se[0], i+se_after_center_included[0]-sz_im[0])
        ...
        for j in range(multivariate_image.shape[1]):
            ...
            se_end_j = max(sz_se[1], j+se_after_center_included[1]-sz_im[1])
            ...
            if selected_idcs.size != 1:
                d_center = se_distances_wrt_center[se_ini_i:se_end_i, se_ini_j:se_end_j].reshape((-1, num_channels))
